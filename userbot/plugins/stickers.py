...
async def _(event):
    ...
    #packshortname = f"Anonymous_{userid}_ns"  # format: Uni_Borg_userid
    if userid == 748471982:
        ...
        if not await stickerset_exists(bot_conv, packshortname):
            ...
            if response.text == FILLED_UP_DADDY:
                while response.text == FILLED_UP_DADDY:
                    ...
                    #if userid == 748471982:
                       # packname = f"{user.first_name}'s Anonymous Vol.{pack}"
                       # packshortname = "Vol._{pack}_Anonymous_ke_locker_me"
                   # else:
                       # packname = f"Vol._{pack}_Anonymous{userid}"
                        #packshortname = f"Vol._{pack}_Anonymous_{userid}_ns"
                    if not await stickerset_exists(bot_conv, packshortname):
